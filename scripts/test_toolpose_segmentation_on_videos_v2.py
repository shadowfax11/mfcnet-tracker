"""
Script for tooltip/pose tracking on videos (per-frame)
...
def main():
    parser = argparse.ArgumentParser(description='Test Toolpose Segmentation Model on Videos')
    ...
    parser.add_argument('--model_type', type=str, required=True,
                        choices=['TernausNet11', 'TernausNet16', 'TAPNet11', 'TAPNet16', 'DeepLab_v3', 'FCN'],
...
def track_on_video(video_path, model, args, logger): 
    model.eval()
    output_fps = 30 
...
def main_worker(args):
    ...
    for video_file in video_files:
        ...
        track_on_video(video_file, model, args, logger)
